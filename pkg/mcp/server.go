...
func (s *Server) handleListTools(req jsonRPCRequest) *jsonRPCResponse {
	tools := []map[string]interface{}{
		...
		// K8s/Helm Management Tools
		{
			"name":        "k8s_list_network_func",
			"description": "List all modifiable core network functions (NF) and their configuration keys.",
			"inputSchema": map[string]interface{}{
				"type":       "object",
				"properties": map[string]interface{}{},
			},
		...
		{
			"name":        "k8s_update_nfconfig",
			"description": "Dynamically update a specific configuration field for a given Network Function.",
			"inputSchema": map[string]interface{}{
				"type": "object",
				"properties": map[string]interface{}{
					"nfName": map[string]string{
						"type":        "string",
						"description": "The name of the NF to modify (e.g., SMF, UPF)",
					},
					"configKey": map[string]string{
						"type":        "string",
						"description": "The configuration key/path to modify (e.g., 'smf_ip_pool', 'upf_routing_rule'). Must be on the allowlist.",
					},
					"newValue": map[string]string{
						"type":        "string",
						"description": "The new value for the configuration key (e.g., '10.0.0.0/24' or JSON string)",
					},
				},
				"required": []string{"nfName", "configKey", "newValue"},
			},
		},
		{
			"name":        "k8s_set_free5gc_helm_base_path",
			"description": "Sets the local absolute path to the free5gc-helm repository directory (required before modifying configs).",
			"inputSchema": map[string]interface{}{
				"type": "object",
				"properties": map[string]interface{}{
					"path": map[string]string{
						"type":        "string",
						"description": "The absolute path to the root of the free5gc-helm folder on the server.",
					},
				},
		...
		{
			"name":        "k8s_start_free5gc",
			"description": "Start free5gc using helm on Kubernetes. This tool will check if K8s is running (and start it if needed), then install or upgrade the free5gc helm chart.",
			"inputSchema": map[string]interface{}{
				"type":       "object",
				"properties": map[string]interface{}{},
			},
		...
		{
			"name":        "k8s_stop_free5gc",
			"description": "Stop free5gc helm deployment on Kubernetes. This will uninstall the free5gc helm release and ensure all pods are terminated. K8s itself will remain running.",
			"inputSchema": map[string]interface{}{
				"type":       "object",
				"properties": map[string]interface{}{},
			},
		...
		{
			"name":        "k8s_free5gc_status",
			"description": "Get the status of all free5gc pods in Kubernetes. Returns detailed information about each pod including their current state, restarts, and age.",
			"inputSchema": map[string]interface{}{
				"type":       "object",
				"properties": map[string]interface{}{},
			},
		...
		{
			"name":        "k8s_upgrade_free5gc",
			"description": "Upgrade the existing free5gc helm deployment. This tool checks if K8s and free5gc are running, then performs a helm upgrade to apply any chart changes. Use this when free5gc-helm configuration has been modified.",
			"inputSchema": map[string]interface{}{
				"type":       "object",
				"properties": map[string]interface{}{},
			},
		...
		{
			"name":        "k8s_start_ueransim",
			"description": "Start UERANSIM using helm on Kubernetes. This tool will check if K8s is running (and start it if needed), then install or upgrade the ueransim helm chart.",
			"inputSchema": map[string]interface{}{
				"type":       "object",
				"properties": map[string]interface{}{},
			},
		...
		{
			"name":        "k8s_stop_ueransim",
			"description": "Stop UERANSIM helm deployment on Kubernetes. This will uninstall the ueransim helm release and ensure all ueransim pods are terminated.",
			"inputSchema": map[string]interface{}{
				"type":       "object",
				"properties": map[string]interface{}{},
			},
		...
		{
			"name":        "k8s_ueransim_status",
			"description": "Get the status of all ueransim pods in Kubernetes. Returns detailed information about each ueransim pod including their current state, restarts, and age.",
			"inputSchema": map[string]interface{}{
				"type":       "object",
				"properties": map[string]interface{}{},
			},
...
func (s *Server) handleCallTool(req jsonRPCRequest) *jsonRPCResponse {
	...
	case "k8s_start_free5gc":
		return s.callK8sStartFree5GC(req.ID)
	case "k8s_stop_free5gc":
		return s.callK8sStopFree5GC(req.ID)
	case "k8s_free5gc_status":
		return s.callK8sFree5GCStatus(req.ID)
	case "k8s_upgrade_free5gc":
		return s.callK8sUpgradeFree5GC(req.ID)
	case "k8s_start_ueransim":
		return s.callK8sStartUeransim(req.ID)
	case "k8s_stop_ueransim":
		return s.callK8sStopUeransim(req.ID)
	case "k8s_ueransim_status":
		return s.callK8sUeransimStatus(req.ID)
	case "k8s_list_network_func":
		return s.callK8sListNetworkFunc(req.ID)
	case "k8s_update_nfconfig":
		return s.callK8sUpdateNFConfig(req.ID, params.Arguments)
	case "k8s_set_free5gc_helm_base_path": // Set free5gc-helm path
		return s.callK8sSetHelmBasePath(req.ID, params.Arguments)
...
// callListNetworkFunc returns a list of NFs and their modifiable config keys.
func (s *Server) callK8sListNetworkFunc(id interface{}) *jsonRPCResponse {
	modifiableConfigs := map[string]interface{}{
		"SMF": map[string]interface{}{
			"description": "Session Management Function",
			"modifiableKeys": []map[string]string{
				{"key": "smf_ip_pool", "type": "string", "example": "10.0.0.0/24", "note": "Defines the subnet for UEs (Requires K8s ConfigMap Update)"},
				// {"key": "log_level", "type": "string", "example": "warn", "note": "Change SMF log verbosity"},
			},
		},
		"UPF": map[string]interface{}{
			"description": "User Plane Function",
			"modifiableKeys": []map[string]string{
				// {"key": "upf_routing_rule", "type": "json_string", "example": `{"match":"all"}`, "note": "Dynamic route entry for PDRs/FARs"},
				{"key": "n4_ip", "type": "string", "example": "192.168.1.100", "note": "N4 interface IP address (Requires restart)"},
			},
		},
		// "AMF": map[string]interface{}{
		// 	"description": "Access and Mobility Management Function",
		// 	"modifiableKeys": []map[string]string{
		// 		{"key": "amf_name", "type": "string", "example": "MyAMF", "note": "Change the AMF human-readable name"},
		// 	},
		// },
	}

	// Convert to JSON string for text response
	jsonBytes, err := json.MarshalIndent(modifiableConfigs, "", "  ")
	if err != nil {
		return s.errorResponse(id, -32603, "failed to marshal network functions", err.Error())
	}
...
// callUpdateNFConfig : handle NF config update request ( now : only for smf ip pool )
func (s *Server) callK8sUpdateNFConfig(id interface{}, args map[string]interface{}) *jsonRPCResponse {
	nfName, _ := args["nfName"].(string)       
	configKey, _ := args["configKey"].(string) // e.g. "smf_ip_pool"
	newValue, _ := args["newValue"].(string)   // e.g. "10.10.0.0/16"

	// check requirement input
	if nfName == "" || configKey == "" || newValue == "" {
		return s.errorResponse(id, -32602, "Missing required parameters", nil)
	}

	// check free5gc-helm path is correct
	if s.client.K8sManager.HelmBasePath == "" {
		return s.errorResponse(id, -32003, "Helm base path not set", "Please run set_free5gc_helm_base_path first.")
	}

	// based on key to modify config
	// now : only support smf ip pool modification
	if configKey == "smf_ip_pool" {
		// target: modify smf AnchorUPF1 ip pool and upf1 ip pool
		smfTargetName := "AnchorUPF1"
		upfTargetName := "upf1"

		// smf yaml path = free5gc-helm/charts/free5gc/chartsfree5gc-smf/values.yaml )
		smfConfigPath := filepath.Join(s.client.K8sManager.HelmBasePath, "charts", "free5gc", "charts", "free5gc-smf", "values.yaml")

		// modify smf yaml file : only modify the upf1
		err := control.ModifySMFValuesYaml(smfConfigPath, smfTargetName, newValue)
		if err != nil {
			return s.errorResponse(id, -32004, fmt.Sprintf("Failed to update SMF CIDR: %v", err), nil)
		}

		// upf yaml path = free5gc-helm/charts/free5gc/charts/free5gc-upf/values.yaml )
		upfConfigPath := filepath.Join(s.client.K8sManager.HelmBasePath, "charts", "free5gc", "charts", "free5gc-upf", "values.yaml")

		err = control.ModifyUPFValuesYaml(upfConfigPath, upfTargetName, newValue)
		
		var upfStatusMsg string // record upf message
		if err != nil {
			// 警告：SMF 成功但 UPF 失敗 (保持 Atomic 意圖，回傳部分成功)
			upfStatusMsg = fmt.Sprintf(" (Warning: SMF updated, but UPF sync failed: %v)", err)
		} else {
			upfStatusMsg = fmt.Sprintf(" and synced to UPF (upf1)")
		}
		...
		return &jsonRPCResponse{
			...
			Result: map[string]interface{}{
				"content": []map[string]string{
					{"type": "text", "text": fmt.Sprintf("Successfully updated %s CIDR to %s in SMF%s.", nfName, newValue, upfStatusMsg)},
				},
				"status": "200 OK",
			},
		}
	...
	return s.errorResponse(id, -32004, "Unsupported configKey for this operation", nil)
}

// callSetHelmBasePath stores the base path for config file operations and validates its existence.
func (s *Server) callK8sSetHelmBasePath(id interface{}, args map[string]interface{}) *jsonRPCResponse {
	path, _ := args["path"].(string)

	if path == "" {
		return s.errorResponse(id, -32602, "missing required parameter: path", nil)
	}

	// check path and is dir or not
	info, err := os.Stat(path)
	if os.IsNotExist(err) {
		return s.errorResponse(id, -32005, "path not found", fmt.Sprintf("Directory '%s' does not exist on the server.", path))
	}
	if err != nil {
		return s.errorResponse(id, -32005, "path access error", err.Error())
	}
	if !info.IsDir() {
		return s.errorResponse(id, -32005, "path is not a directory", fmt.Sprintf("Path '%s' is a file, not a directory.", path))
	}

	// check the struct of free5gc-helm：[path]/charts/free5gc/
	expectedSubDir := filepath.Join(path, "charts", "free5gc")
	_, err = os.Stat(expectedSubDir)
	if os.IsNotExist(err) {
		return s.errorResponse(id, -32005, "validation failed", fmt.Sprintf("Directory '%s' does not appear to be the correct free5gc-helm root (missing required subdirectory: %s).", path, expectedSubDir))
	}
	if err != nil {
		return s.errorResponse(id, -32005, "validation error", err.Error())
	}

	s.client.K8sManager.HelmBasePath = path
	...
	return &jsonRPCResponse{
		...
		Result: map[string]interface{}{
			"content": []map[string]string{
				{"type": "text", "text": fmt.Sprintf("Helm base path successfully validated and set to: %s. Configuration files can now be modified.", path)},
			},
			"status": "200 OK",
		},
	}
...
func (s *Server) callK8sStartFree5GC(id interface{}) *jsonRPCResponse {
	err := s.client.StartFree5GCHelm()
	if err != nil {
		return s.errorResponse(id, -32001, "failed to start free5gc helm", err.Error())
	}

	// Get status after starting
	status, statusErr := s.client.GetFree5GCHelmStatus()
	if statusErr != nil {
		status = fmt.Sprintf("Started successfully but failed to get status: %v", statusErr)
	}
	...
	return &jsonRPCResponse{
		...
		ID:      id,
		Result: map[string]interface{}{
			"content": []map[string]string{{"type": "text", "text": fmt.Sprintf("free5gc helm chart started successfully\n\nPod Status:\n%s", status)}},
		},
	}
}

func (s *Server) callK8sStopFree5GC(id interface{}) *jsonRPCResponse {
	err := s.client.StopFree5GCHelm()
	if err != nil {
		return s.errorResponse(id, -32001, "failed to stop free5gc helm", err.Error())
	}
	...
	return &jsonRPCResponse{
		...
		ID:      id,
		Result: map[string]interface{}{
			"content": []map[string]string{{"type": "text", "text": "free5gc helm chart stopped successfully. All pods have been terminated."}},
		},
	}
}

func (s *Server) callK8sFree5GCStatus(id interface{}) *jsonRPCResponse {
	status, err := s.client.GetFree5GCHelmStatus()
	if err != nil {
	...
	return &jsonRPCResponse{
		...
		ID:      id,
		Result: map[string]interface{}{
			"content": []map[string]string{{"type": "text", "text": fmt.Sprintf("free5gc Pod Status:\n\n%s", status)}},
		},
	}
}

func (s *Server) callK8sUpgradeFree5GC(id interface{}) *jsonRPCResponse {
	err := s.client.UpgradeFree5GCHelm()
	if err != nil {
		return s.errorResponse(id, -32001, "failed to upgrade free5gc helm", err.Error())
	}

	// Get status after upgrade
	status, statusErr := s.client.GetFree5GCHelmStatus()
	if statusErr != nil {
		status = fmt.Sprintf("Upgraded successfully but failed to get status: %v", statusErr)
	}
	...
	return &jsonRPCResponse{
		...
		ID:      id,
		Result: map[string]interface{}{
			"content": []map[string]string{{"type": "text", "text": fmt.Sprintf("free5gc helm chart upgraded successfully\n\nPod Status:\n%s", status)}},
		},
	}
}

func (s *Server) callK8sStartUeransim(id interface{}) *jsonRPCResponse {
	err := s.client.StartUeransimHelm()
	if err != nil {
		return s.errorResponse(id, -32001, "failed to start ueransim helm", err.Error())
	}

	// Get status after starting
	status, statusErr := s.client.GetUeransimHelmStatus()
	if statusErr != nil {
		status = fmt.Sprintf("Started successfully but failed to get status: %v", statusErr)
	}
	...
	return &jsonRPCResponse{
		...
		ID:      id,
		Result: map[string]interface{}{
			"content": []map[string]string{{"type": "text", "text": fmt.Sprintf("ueransim helm chart started successfully\n\nPod Status:\n%s", status)}},
		},
	}
}

func (s *Server) callK8sStopUeransim(id interface{}) *jsonRPCResponse {
	err := s.client.StopUeransimHelm()
	if err != nil {
		return s.errorResponse(id, -32001, "failed to stop ueransim helm", err.Error())
	}
	...
	return &jsonRPCResponse{
		...
		ID:      id,
		Result: map[string]interface{}{
			"content": []map[string]string{{"type": "text", "text": "ueransim helm chart stopped successfully. All ueransim pods have been terminated."}},
		},
	}
}

func (s *Server) callK8sUeransimStatus(id interface{}) *jsonRPCResponse {
	status, err := s.client.GetUeransimHelmStatus()
	if err != nil {
		return s.errorResponse(id, -32001, "failed to get ueransim status", err.Error())
	}
	...
	return &jsonRPCResponse{
		...
		ID:      id,
		Result: map[string]interface{}{
			"content": []map[string]string{{"type": "text", "text": fmt.Sprintf("UERANSIM Pod Status:\n\n%s", status)}},
		},
	}
}
